...
if (isset($_POST['dathang'])) {
    ...
    $tongtien = $allMoney;  // Tổng tiền chưa giảm
    $tongtien_saugiam = $tongtien - $voucher_discount;  // Tổng tiền sau giảm

    $ID_ThanhVien = $_GET['id'];

    mysqli_query($mysqli, $update_sql);

    // Cập nhật lại số lượng voucher nếu cần
    if ($voucher_code) {
        $update_voucher = "UPDATE voucher SET SoLuong = SoLuong - 1 WHERE MaVoucher = '$voucher_code'";
        mysqli_query($mysqli, $update_voucher);
    }
                        ...
                        <table class="table">
                            ...
                            if (isset($_SESSION['cart'])) {
                                ...
                                if (isset($_POST['voucher_code']) && $_POST['voucher_code'] != "") {
                                    $voucher_code = mysqli_real_escape_string($mysqli, $_POST['voucher_code']);
                                    $sql_voucher = "SELECT * FROM voucher WHERE MaVoucher = '$voucher_code' AND HanSuDung >= CURDATE() AND SoLuong > 0";
                                    $result_voucher = mysqli_query($mysqli, $sql_voucher);
                                    if (mysqli_num_rows($result_voucher) > 0) {
                                        $row_voucher = mysqli_fetch_assoc($result_voucher);
                                        if ($row_voucher['Loai'] == 'percent') {
                                            $voucher_discount = $allMoney * ($row_voucher['GiaTri'] / 100);
                                        } else {
                                            $voucher_discount = $row_voucher['GiaTri'];
                                        }
                                        if ($voucher_discount > $allMoney) {
                                            $voucher_discount = $allMoney;
                                        }
                                ...
                                // Lưu tổng tiền sau khi áp dụng voucher vào CSDL
                                $totalAfterDiscount = $allMoney - $voucher_discount;
                                if (isset($_POST['dathang'])) {
                                    $ID_ThanhVien = $_GET['id'];
                                    ...
                                    if (isset($_POST['voucher_code']) && $_POST['voucher_code'] != "") {
                                        ...
                                        if (mysqli_num_rows($result_voucher) > 0) {
                                            ...
                                            // Không cho giảm nhiều hơn tổng tiền
                                            if ($voucher_discount > $allMoney) {
                                                $voucher_discount = $allMoney;
                                            }

                                            // Lưu session để dùng khi đặt hàng
                                            $_SESSION['voucher_code'] = $row_voucher['ID_Voucher'];
                                            $_SESSION['voucher_discount'] = $voucher_discount;
                                        } else {
                                            echo "<script>alert('Voucher không hợp lệ hoặc đã hết hạn!');</script>";
                                        }
                            ...
                            <div class="total-price">
                                <?php if ($voucher_discount > 0): ?>
                                    ...
                                    <h5 class="final-price">Tổng tiền sau giảm
                                        giá: <?= number_format($allMoney - $voucher_discount) ?> Đồng</h5>
                                <?php else: ?>
                                    <div class="normal-price">
                                        <h5>Tổng tiền: <?= number_format($allMoney) ?> Đồng</h5>
                                    </div>
...
<style>
    ...
    .final-price {
        font-size: 24px;
        font-weight: bold;
        color: #248A32; /* Màu xanh đặc trưng */
        background-color: #f0f8f0; /* Nền sáng nhẹ để làm nổi bật */
        padding: 10px;
        border-radius: 5px;
        text-align: center;
        margin-top: 15px;
        box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); /* Đổ bóng nhẹ để nổi bật hơn */
    }
    ...
    .total-price .normal-price {
        font-size: 20px;
        font-weight: 600;
        color: #555; /* Màu xám nhạt cho tổng tiền ban đầu */
        padding: 8px 10px;
        background-color: #f9f9f9;
        border-radius: 5px;
        text-align: center;
    ...
    .total-price .normal-price strong {
        color: #248A32; /* Màu xanh cho số tiền */
    }
    ...
    .total-price .discounted-price {
        font-size: 22px;
        font-weight: 600;
        color: #d9534f; /* Màu đỏ cho phần giảm giá */
        padding: 10px 15px;
